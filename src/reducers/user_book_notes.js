import {
  FETCH_USER_BOOK_NOTES_PENDING,
  FETCH_USER_BOOK_NOTES_FULFILLED,
  FETCH_USER_BOOK_NOTES_REJECTED,
  POST_NOTE_FULFILLED,
...
export default function user_book_notes(state = initialState, action = {}) {
  switch (action.type) {

    case FETCH_USER_BOOK_NOTES_PENDING:
      console.log('fetching user book notes');
      console.log(action);
      return Object.assign({}, state, { fetching_notes: true });

    case FETCH_USER_BOOK_NOTES_FULFILLED:
      console.log('fetch user book notes success');
      console.log(action);
    ...
    case POST_NOTE_FULFILLED:
      ...
      if (state.notes.length && (state.notes[0].book == action.payload.book)) {
        console.log('add note in user book notes');
    ...
    case DELETE_NOTE_FULFILLED:
      ...
      const notes_del = state.notes.filter(item => {
        if (action.meta.noteID == item.uuid) {
          console.log('delete note in user book notes');
          return false;
        }
        return true;
      });
    ...
    case TOGGLE_FAVOR_OF_NOTE_FULFILLED:
      ...
      if (rateNoteId != -1) {
        console.log('toggle favor in user book notes');
