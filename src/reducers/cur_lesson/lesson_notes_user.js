import {
  FETCH_LESSON_USER_NOTES_PENDING,
  FETCH_LESSON_USER_NOTES_FULFILLED,
  FETCH_LESSON_USER_NOTES_REJECTED,
  POST_NOTE_FULFILLED,
...
export default function lesson_notes_user(state = initialState, action = {}) {
  switch (action.type) {

    case FETCH_LESSON_USER_NOTES_PENDING:
      console.log('fetching lesson user notes');
      console.log(action);
      return Object.assign({}, state, { fetching_notes: true });

    case FETCH_LESSON_USER_NOTES_FULFILLED:
      console.log('fetch lesson user notes success');
      console.log(action);
    ...
    case POST_NOTE_FULFILLED:
      ...
      if (state.notes.length && (state.notes[0].lesson_page.split('/')[0] == action.payload.lesson_page.split('/')[0])) {
        console.log('add note in lesson user notes');
    ...
    case DELETE_NOTE_FULFILLED:
      ...
      const notes_del = state.notes.filter(item => {
        if (action.meta.noteID == item.uuid) {
          console.log('delete note in lesson user notes');
          return false;
        }
        return true;
      });
    ...
    case TOGGLE_FAVOR_OF_NOTE_FULFILLED:
      ...
      if (rateNoteId != -1) {
        console.log('toggle favor in lesson user notes');
