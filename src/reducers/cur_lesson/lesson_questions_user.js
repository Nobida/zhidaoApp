import {
  FETCH_LESSON_USER_QUESTIONS_PENDING,
  FETCH_LESSON_USER_QUESTIONS_FULFILLED,
  FETCH_LESSON_USER_QUESTIONS_REJECTED,
...
export default function lesson_questions_user(state = initialState, action = {}) {
  switch (action.type) {

    case FETCH_LESSON_USER_QUESTIONS_PENDING:
      console.log('fetching questions');
      console.log(action);
      return Object.assign({}, state, { fetching_questions: true });

    case FETCH_LESSON_USER_QUESTIONS_FULFILLED:
    ...
    case POST_QUESTION_FULFILLED:
      ...
      if (state.questions.length && (state.questions[0].lesson_page.split('/')[0] == action.payload.lesson_page.split('/')[0])) {
        console.log('add question in lesson user questions');
    ...
    case DELETE_QUESTION_FULFILLED:
      ...
      const questions_del = state.questions.filter(item => {
        if (action.meta.questionID == item.uuid) {
          console.log('delete question in lesson user questions');
          return false;
        }
        return true;
      });
