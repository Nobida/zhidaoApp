...
import { ROOT_URL,BASIC_URL,FOLLOW_URL,DEFAULT_CARD_BG } from "../../../api/const";
...
export default class DailyCard extends React.Component {
  ...
  drawImage(imgSrc,imgInfo,title,desc,course,day_num,lesson_id,user_id){
    ...
    let textFont_small = '200 15px Microsoft Yahei';
    ...
    followUrl+='?lesson='+lesson_id+'&user='+user_id+'&source=daily_memo';
    let footerSrc = 'img/share-footer.png';
    ...
    function drawDayInfo(){
      ...
      ctx.font = dayFont;
      ctx.fillText(day_num,width/2,day_info_h +10);
      ctx.strokeStyle = '#f8f8f8';
      ctx.beginPath();
      ctx.moveTo(20,day_info_h);
      ctx.lineTo((width-logoSize)/2 - 50,day_info_h);
      ctx.stroke();
      ctx.closePath();

      ctx.beginPath();
      ctx.moveTo((width+logoSize)/2 + 50,day_info_h);
    ...
    if(desc){
      ...
      if(lines.length==2) {
        if(lines[0].length>10 || lines[1].length>10){
          ctx.font = textFont_small;
        }
  ...
  componentDidMount(){
    ...
    const lesson_id = this.props.lesson_info.id;
    if(lesson_id) {
      const user_id = this.props.user_id;
      const {picture,day_num,name,desc,picture_caption,course} = this.props.lesson_info;
      let desc_part = desc.split('\n')
      let time = desc_part[0]
      let lunar_time = desc_part[1]
      let desc_line = desc_part[2]
      let from = desc_part[3]
      this.drawImage(picture?ROOT_URL+picture:DEFAULT_CARD_BG,picture_caption,from,desc_line,course,lunar_time,lesson_id,user_id);
  ...
  componentWillReceiveProps(nextProps){
    ...
    if(cur_lesson_id && lesson_id != cur_lesson_id) {
      ...
      let {picture,day_num,name,desc,picture_caption,course} = nextProps.lesson_info;
      let desc_part = desc.split('\n')
      let time = desc_part[0]
      let lunar_time = desc_part[1]
      let desc_line = desc_part[2]
  ...
  render() {
    ...
    let loaded= (this.state.headImageLoaded && this.state.footerImageLoaded)
    return (
