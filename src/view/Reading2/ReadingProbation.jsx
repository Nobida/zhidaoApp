...
export default class Reading extends React.Component {
  ...
  componentDidMount() {
    ...
    if (lesson_id && this.mismatchLessonID()) {
      ...
      actions.clearLesson();
      actions.fetchPublicLesson(lesson_id,course_id);
  ...
  render() {
    ...
    const { reading_settings,notes_user,location, agent_type, cur_lesson,match,cur_course } = this.props;
    const lesson_info = cur_lesson.lesson_info.lesson_info;
    const lesson_id = match.params.id;
    ...
    const unicomList = location.pathname.split("/")
    console.log("unicomList")
    const ifUnicom = (unicomList[1]==='reading-probation-unicom'?true:false)





    const part_info_unicom = part_info!==null?part_info.filter(function(d){return d.id!=='PREVIEW'&&d.id!=='REVIEW'}):null
    console.log(part_info_unicom)


    const a = part_info!==null?part_info[part_info_unicom.length].id:null
    console.log(a)
    ...
    return (
      <div className={"reading theme-"+reading_settings.theme}>
        ...
        /> 
        {part_info!==null&&ifUnicom&&unicomList[unicomList.length-1]!==part_info[part_info_unicom.length].id?(
          <div className='sign-footer-unicom'>
            <Button type='primary' size='lg' className="fetchMoreClasses" onClick={this.showShareLead.bind(this)}>获取更多课程</Button>
          </div>):(part_info!==null&&ifUnicom==false?(
          <div className='sign-footer'>
            <a href={cur_course.course && cur_course.course.ad_url?cur_course.course.ad_url:''}><Button type='primary'>立即报名</Button></a>
          </div>):(<div className='sign-footer-unicom-none'></div>))
        } 
